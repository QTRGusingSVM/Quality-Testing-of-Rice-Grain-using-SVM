function [f,Original]=SubRutineIM(vid,XVid,sizeKernel,Color,DiskSize)
...
for al=1:numLabels
    for it=(20*al+1):(20*al+20)
        for jt=1:20
            A= cell2mat(AC(1,al));
            videoFrame(it,jt,1)=A(1,1);
            if A>=Color
               videoFrame(it,jt,2)=A(1,2);
            else
            videoFrame(it,jt,2)=1;
            end
            videoFrame(it,jt,3)=A(1,3);
        end
    end
end
